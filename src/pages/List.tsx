...
const List: React.FC = () => {
  ...
  useIonViewWillEnter( async () => {
    ...
    console.log(' ~ file: List.tsx:80 ~ useIconViewWillEnter ~ users', users);
    setUsers(users);
    setLoading(false);
  });
  ...
  return (
    <IonPage ref={page}>
      ...
      <IonContent>
        ...
        <IonModal breakpoints={[0, 0.5, 0.8]} initialBreakpoint={0.5} ref={modal} isOpen={selectedUser !== null} onDidDismiss={() => setSelectedUser(null)}>
          <IonHeader>
          ...
          <IonContent className='ion-padding'>
            {activeSegment === 'details' && (
              <IonCard>
                ...
                <IonCardContent className='ion-no-padding'>
                  <IonItem lines='none'>
                    <IonLabel className='ion-text-wrapping'>
                      {selectedUser?.name.first} {selectedUser?.name.last}
